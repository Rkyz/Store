  ...
  return (
    <div className={`h-[auto] w-[100%] justify-center flex gap-x-5  flex-wrap  ${CardList ? ' gap-y-5' : 'gap-y-20'}`}>
      {Pcgame.length === 0 ? (
        ...
        Pcgame.map(game => (
          <div key={game.id} className={`bg-slate-500 flex rounded-lg ${CardList ? 'w-[88%] max-lg:w-full flex-row h-[65px] items-center px-3' : 'w-[160px] h-[219px] items-center flex-col py-6 justify-between relative mt-20'}`}>
            <div className={`${CardList ? 'flex items-center' : ''}`}>
              ...
              <ul className={`flex flex-col ${CardList ? '' : '-translate-y-12 w-[116px] text-center items-center'}`}>
                <p className="text-white text-xs">{game.creator}</p>
                <p className="text-base text-white">{game.name}</p>
              </ul>
            </div>
