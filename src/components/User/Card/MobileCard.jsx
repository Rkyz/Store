    ...
    <div className={`h-[auto] w-[100%] justify-center flex gap-x-5  flex-wrap  ${CardList ? ' gap-y-5' : 'gap-y-20'}`}>
      {Mobilegame.length === 0 ? (
        <div className='my-32'>
          ...
          <div className='flex justify-center'>
            <p className='animate-bganimate dark:text-white text-black text-opacity-80 '>No Mobile Game Data Available</p>
          </div>
        </div>
      ) : (
        Mobilegame.map(game => (
          <div key={game.id} className="w-[148px] relative h-[219px] bg-lightmode dark:bg-slate-500 px-20 mt-20 flex flex-col items-center rounded-lg justify-between mb-4">
            <div className='flex flex-col gap-y-3'>
              ...
              <ul className="-translate-y-9 w-[116px] flex flex-col items-center justify-start">
                ...
                <p className={`text-center text-gray-400 dark:text-white ${game.type === 'joki' ? 'text-xs mt-1' : 'text-base'}`}>{game.type === 'joki' ? `Joki ${game.name}` : game.name}</p>
              </ul>
            </div>
            {game.type === 'joki' && (
