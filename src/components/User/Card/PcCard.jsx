  ...
  const getDataCard = () => {
    ...
    axios
      ...
      .then(response => {
        ...
        const pcGamesData = games.filter(game => game.category === 'pc'); 
        setPcgame(pcGamesData); 
      })
      .catch(error => {
    ...
    <div className={`h-[auto] w-[100%] justify-center flex gap-x-5  flex-wrap  ${CardList ? ' gap-y-5' : 'gap-y-20'}`}>
      {Pcgame.length === 0 ? (
        <div className='my-32'>
          ...
          <div className='flex justify-center'>
            <p className='animate-bganimate dark:text-white text-black text-opacity-80  '>No Pc Game Data Available</p>
          </div>
        </div>
      ) : (
        Pcgame.map(game => (
          <div key={game.id} className={`dark:text-white dark:bg-slate-500 bg-lightmode text-gray-400 flex rounded-lg ${CardList ? 'w-[88%] max-lg:w-full flex-row h-[65px] items-center px-3' : 'w-[160px] h-[219px] items-center flex-col py-6 justify-between relative mt-20'}`}>
            <div className={`${CardList ? 'flex items-center' : ''}`}>
              ...
              <ul className={`flex flex-col ${CardList ? '' : '-translate-y-12 w-[116px] text-center items-center'}`}>
                ...
                <p className="text-base text-gray-400 dark:text-white">{game.name}</p>
              </ul>
            </div>
