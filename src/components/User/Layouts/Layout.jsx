                ...
                <ul className='flex flex-col w-full max-lg:items-center max-lg:flex-row max-lg:justify-center z-30'>
                    <li className='flex gap-4 items-center justify-between h-[100px] p-8 max-lg:hidden'>
                        <button onClick={toggleExpansion}>
                            <GiHamburger className={`text-3xl ${expanded ? 'dark:text-white text-black opacity-80' : 'dark:text-white text-black opacity-80'}`} />
                        </button>
                        <p className={`dark:text-white text-black opacity-80 ${showLogo ? 'block' : 'hidden'}`}>Logo</p>
                    </li>
                    <li className={`h-[70px] flex items-center lg:px-8 max-lg:p-5 md:p-9 ${expanded ? 'max-lg:hidden' : ''} ${currentPath === '/' ? 'dark:bg-blue-500 bg-yellow-500  text-white max-lg:rounded-full max-lg:w-[65px] max-lg:h-[65px]' : ''}`}>
                    ...
                    <li className={`h-[70px] flex items-center lg:px-8 max-lg:p-5 md:p-9 ${expanded ? 'max-lg:hidden' : ''}  ${currentPath === '/promo' ? 'dark:bg-blue-500 bg-yellow-500    text-white max-lg:rounded-full max-lg:w-[65px] max-lg:h-[65px]' : ''}`}>
                    ...
                    <li className={`h-[70px] flex items-center lg:px-8 max-lg:p-5 md:px-9 ${expanded ? 'max-lg:hidden' : ''} ${currentPath === '/game' ? 'dark:bg-blue-500 bg-yellow-500     text-white max-lg:rounded-full max-lg:w-[65px] max-lg:h-[65px]' : ''}`}>
                    ...
                    <li className={`h-[7vh] flex items-center lg:px-8 max-lg:p-5 md:px-9 lg:hidden ${expanded ? 'max-lg:hidden' : ''}`}>
                        <button onClick={toggleExpansion}>
                            <GiHamburger className={`text-3xl ${expanded ? 'text-white' : 'text-black dark:text-white'}`} />
                        </button>
                    ...
                    <div className={`absolute max-lg:overflow-hidden lg:relative max-lg:backdrop-blur-3xl max-lg:bg-Darkmode max-lg:z-50 max-lg:text-white max-lg:rounded-ss-[2rem] max-lg:rounded-se-[2rem] ${expanded ? 'h-auto max-lg:w-[32rem] max-md:w-[96%] bottom-0 pb-10' : ' hidden'}`} style={{ backdropFilter: expanded ? 'none' : 'blue(10px)' }}>
                        ...
                        <li className={`h-[7vh] flex items-center px-8 ${expanded ? '' : 'bg-transparent'} ${currentPath === '/hitungwr' && expanded ? 'dark:bg-blue-500 bg-gray-400 text-white' : ''}`}>
                        ...
                        <li className={`h-[7vh] flex items-center px-8 ${expanded ? '' : 'bg-transparent'} ${currentPath === '/hitungzodiac' && expanded ? 'dark:bg-blue-500 bg-gray-400 text-white' : ''}`}>
